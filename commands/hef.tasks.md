...
3. **Task format rules**:
   ...
   - A task that implements an FR must produce a test that **cites that FR id** (marker, describe
     title, or comment); `/hef.verify` maps requirements to tests by that token, so a task whose
...
5. **Present summary** to user:
   ...
   - Suggest next step: `/hef.implement` or `/hef.checklist` for pre-implementation validation;
     after implementation, `/hef.verify` closes the loop
